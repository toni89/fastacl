...
class FastACL {
  ...
  middleware(options) {
    ...
    return function(req, res, next) {
      if(!this.checkRoute(get(req, options.roles, [], get(req, 'route.url') , get(req, 'route.method')))) {
        ...
        if(rule.rules && rule[action] && Array.isArray(rule[action])) {
          for(let details of rule[action]) {
            if(details.hasOwnProperty("scope") && details.hasOwnProperty("permissions")){
              ...
              self[action](rule.rules, details.route, details.methods.map(m => m.toUpperCase()), 'routes');
            }
          }
        }
      }
